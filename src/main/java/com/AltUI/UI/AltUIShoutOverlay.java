...
public class AltUIShoutOverlay extends Overlay {
    ...
    @Inject
    public AltUIShoutOverlay(AltUIPlugin plugin, int viewportWidth, int viewportHeight) {
        super(plugin);
    ...
    public Dimension render(Graphics2D graphics) {
        ...
        if(thingsToBeFussedAbout[AltUIShoutOverlay.GO_BANK]) {
            graphics.setColor(Color.BLACK);
            graphics.drawString("Go bank",x + w/2 - metrics.stringWidth("Go bank")/2,y + metrics.getAscent() + 5);
        }
        if(thingsToBeFussedAbout[AltUIShoutOverlay.TOO_AFK]) {
            graphics.setColor(Color.BLACK);
        ...
        if(thingsToBeFussedAbout[AltUIShoutOverlay.SHARD_NEARBY] && blinker % flashInterval > flashInterval/3) {
            int shardW = 150;
